...
public class PrintUtils {
    public void printEvent(Fish male, Fish female, int maleChild, int femaleChild) {
        System.out.printf(
                "%d yoshli \"%s\" erkak baliq -> %d yoshli \"%s\" urg'ochi baliq turmush qurishdi %n" +
                        "  ==>   %d ta og'il va %d ta qiz farzand%n",
                male.getAge(),
                male.getName().toUpperCase(),
                female.getAge(),
                female.getName().toUpperCase(),
                maleChild,
                femaleChild);
    ...
    public void printDead(Fish fish) {
        System.out.printf("\"%s\" ismli baliq %d yoshida vafot etdi \n", fish.getName(), fish.getMaxAge());
    }
